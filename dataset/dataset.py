...
class ConformerSpecAugment(torch.nn.Module):
    ...
    def forward(self, spec):
        ...
        for _ in range(self.num_time_masks):
            num_frames = out.size(-1)
            max_width = self.time_mask_param
            if max_width is None:
                max_width = max(1, int(num_frames * self.time_mask_max_ratio))
            width = int(torch.randint(0, max_width + 1, (1,)).item())
            if width == 0 or width >= num_frames:
                continue
            start = int(torch.randint(0, num_frames - width + 1, (1,)).item())
...
class LibriSpeechDataset(Dataset):
    ...
    def __init__(
        ...
        self.audio2mels = T.MelSpectrogram(
            ...
            n_fft=512,
            win_length=int(sampling_rate * 0.025),
    ...
    def __getitem__(self, idx):
        ...
        if self.num_audio_channels == 1 and audio.size(0) > 1:
