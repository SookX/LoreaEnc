...
run_ft() {
    ...
    log_phase "FT ${mode}_${hours}h  |  ${EPOCHS} epochs  |  SSL=${ssl_ckpt}  |  recipe=simple"
    export MASTER_PORT=$((PORT_BASE + 200 + (hours == 10 ? 1 : 2)))
    # Simple fine-tune recipe (matches the fair 100h baseline). IDENTICAL
    # across all three codebook conditions — only the SSL pretraining
    # objective varies.
    torchrun \
        --nproc_per_node=4 \
        --master_addr="${MASTER_ADDR}" \
