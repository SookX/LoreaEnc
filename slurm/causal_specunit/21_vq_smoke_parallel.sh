#!/bin/bash
# VQ smoke test: parallel learned VQ (two independent codebooks K1=100, K2=500)
# on SqueezeFormer-XS. The strict learned analogue of the parallel k-means
# dual codebook in our main recipe.
#
# Self-contained: from a trained parallel-VQ quantizer through to
# test-clean/test-other WER numbers. Stages gated by output existence so
# re-submission resumes.
#
# Pipeline:
#   A. Generate VQ targets (z100=codebook-1 indices, z500=codebook-2 indices)
#   B. Shard targets
#   C. SSL pretrain 150k steps, --codebook-mode both K_c=100 K_f=500
#   D. Fine-tune on Libri-Light 10h, seed 42
#   E. Evaluate on test-clean / test-other
#
# Prerequisite: outputs/causal_specunit/vq/parallel_100_500/state.pt
# Run slurm/causal_specunit/20_train_quantizer.sh with QUANTIZER_TYPE=parallel K1=100 K2=500 first.
#
# Submit:
#   sbatch slurm/causal_specunit/21_vq_smoke_parallel.sh

#SBATCH --partition=common
#SBATCH --qos=bg-eng-01
#SBATCH --account=bg-eng-01
#SBATCH --job-name=vq_par
...
#SBATCH --mem=256G
#SBATCH --gres=gpu:2
#SBATCH -o /valhalla/projects/bg-eng-01/LoreaEnc/logs/vq_par.%j.out
#SBATCH -e /valhalla/projects/bg-eng-01/LoreaEnc/logs/vq_par.%j.err
...
QUANTIZER_DIR="outputs/causal_specunit/vq/parallel_100_500"
TARGETS_DIR="outputs/causal_specunit/targets_960h_c8_parallel_100_500"
SSL_OUTPUT_DIR="outputs/causal_specunit/ssl_parallel_iter1_150k"
FT_OUTPUT_ROOT="outputs/causal_specunit/vq_smoke/parallel"
CODEBOOK_MODE="both"
K_COARSE="100"
K_FINE="500"
...
[ -f "${QUANTIZER_DIR}/state.pt" ]               || { echo "Missing quantizer: ${QUANTIZER_DIR}/state.pt. Run 20_train_quantizer.sh with QUANTIZER_TYPE=parallel K1=100 K2=500 first."; exit 1; }
...
PORT_BASE=$((37000 + (SLURM_JOB_ID % 2500)))

# ==========================================================
# Stage A: Generate VQ targets (two-stream parallel)
# ==========================================================
if [ ! -f "${TARGETS_DIR}/targets.pt" ]; then
    log_phase "Stage A: generate VQ targets (quantizer=${QUANTIZER_TYPE} K1=100 K2=500, two-stream parallel)"
...
# ==========================================================
# Stage C: SSL pretrain on parallel-VQ targets
# ==========================================================
...
log_phase "DONE VQ smoke (${QUANTIZER_TYPE} K1=100 K2=500)"
