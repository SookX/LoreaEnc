...
#SBATCH --job-name=csu_ctc100h_aug_fair
...
#SBATCH --mem=256G
#SBATCH --gres=gpu:2
#SBATCH -o /valhalla/projects/bg-eng-01/LoreaEnc/logs/csu_ctc100h_aug_fair.%j.out
#SBATCH -e /valhalla/projects/bg-eng-01/LoreaEnc/logs/csu_ctc100h_aug_fair.%j.err
...
# Same fair optimizer structure as the 10h fair run: both scratch and SSL use
# encoder/head parameter groups. The intended difference is only SSL init.
...
# Match the 10h fair recipe by default. Override these if you explicitly want
# the stronger 100h recipe, e.g. SPECAUG_TIME_MASK_PARAM=40 ...
...
SCRATCH_OUTPUT_DIR="${SCRATCH_OUTPUT_DIR:-outputs/causal_specunit/ctc_scratch_100h_specaug_fair_elr3e4_hlr1e3_w10_p50_100ep_c8}"
SSL_OUTPUT_DIR="${SSL_OUTPUT_DIR:-outputs/causal_specunit/ctc_ssl_100h_specaug_fair_elr3e4_hlr1e3_w10_p50_100ep_c8}"
...
echo "Job ${SLURM_JOB_ID} fair 100h SpecAug CTC comparison starting at $(date)"
...
echo "Starting fair scratch 100h SpecAug run at $(date)"
torchrun \
...
echo "Starting fair SSL 100h SpecAug run at $(date)"
...
echo "Job ${SLURM_JOB_ID} fair 100h SpecAug CTC comparison finished at $(date)"
echo "Metrics:"
