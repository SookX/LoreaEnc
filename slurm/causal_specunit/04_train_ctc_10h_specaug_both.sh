...
#SBATCH --job-name=csu_ctc10h_aug_both
...
#SBATCH --mem=160G
#SBATCH --gres=gpu:2
#SBATCH -o /valhalla/projects/bg-eng-01/LoreaEnc/logs/csu_ctc10h_aug_both.%j.out
#SBATCH -e /valhalla/projects/bg-eng-01/LoreaEnc/logs/csu_ctc10h_aug_both.%j.err
...
SSL_ENCODER_LR="${SSL_ENCODER_LR:-2e-4}"
SSL_HEAD_LR="${SSL_HEAD_LR:-1e-3}"
SSL_BASE_LR="${SSL_BASE_LR:-1e-3}"
WARMUP_EPOCHS="${WARMUP_EPOCHS:-5}"
PEAK_EPOCHS="${PEAK_EPOCHS:-35}"
NOAM_DECAY_RATE="${NOAM_DECAY_RATE:-0.5}"
MAX_GRAD_NORM="${MAX_GRAD_NORM:-1.0}"

# Mild SpecAugment for 10h. This keeps the comparison strong without burying the
# small supervised set under the 100h recipe's heavier masks.
...
SPECAUG_DISABLE_LAST_EPOCHS="${SPECAUG_DISABLE_LAST_EPOCHS:-20}"

SCRATCH_OUTPUT_DIR="${SCRATCH_OUTPUT_DIR:-outputs/causal_specunit/ctc_scratch_10h_specaug_w5_p35_150ep_c8}"
SSL_OUTPUT_DIR="${SSL_OUTPUT_DIR:-outputs/causal_specunit/ctc_ssl_10h_specaug_elr2e4_hlr1e3_w5_p35_150ep_c8}"
...
echo "Job ${SLURM_JOB_ID} 10h SpecAug CTC comparison starting at $(date)"
...
echo "Scratch LR: ${SCRATCH_LR}"
echo "SSL LRs: encoder=${SSL_ENCODER_LR} head=${SSL_HEAD_LR} base=${SSL_BASE_LR}"
...
echo "Starting scratch 10h SpecAug run at $(date)"
torchrun \
...
echo "Starting SSL 10h SpecAug run at $(date)"
...
echo "Job ${SLURM_JOB_ID} 10h SpecAug CTC comparison finished at $(date)"
echo "Metrics:"
