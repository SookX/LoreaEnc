#!/bin/bash
# VQ smoke test: flat learned VQ K=600 on SqueezeFormer-XS.
#
# Self-contained: from a trained VQ-600 quantizer through to test-clean/test-other
# WER numbers. Stages gated by output-file existence so re-submission resumes.
#
# Single-stream variant: the VQ produces one index per chunk. We write the
# target file with z500=indices, z100=zeros (dummy, never predicted because
# SSL is launched with --codebook-mode fine).
#
# Pipeline:
#   A. Generate VQ targets
#   B. Shard targets
#   C. SSL pretrain 150k steps, --codebook-mode fine --k-fine 600
#   D. Fine-tune on Libri-Light 10h, seed 42
#   E. Evaluate on test-clean / test-other
#
# Prerequisite: outputs/causal_specunit/vq/vq_600/state.pt
# Run slurm/causal_specunit/20_train_quantizer.sh with QUANTIZER_TYPE=vq K1=600 first.
#
# Submit:
#   sbatch slurm/causal_specunit/21_vq_smoke_vq_600.sh

#SBATCH --partition=common
#SBATCH --qos=bg-eng-01
#SBATCH --account=bg-eng-01
#SBATCH --job-name=vq_vq600
...
#SBATCH --mem=256G
#SBATCH --gres=gpu:2
#SBATCH -o /valhalla/projects/bg-eng-01/LoreaEnc/logs/vq_vq600.%j.out
#SBATCH -e /valhalla/projects/bg-eng-01/LoreaEnc/logs/vq_vq600.%j.err
...
QUANTIZER_TYPE="vq"
QUANTIZER_DIR="outputs/causal_specunit/vq/vq_600"
TARGETS_DIR="outputs/causal_specunit/targets_960h_c8_vq_600"
SSL_OUTPUT_DIR="outputs/causal_specunit/ssl_vq600_iter1_150k"
FT_OUTPUT_ROOT="outputs/causal_specunit/vq_smoke/vq_600"
CODEBOOK_MODE="fine"
K_COARSE="100"   # dummy: targets carry z100=zeros, never predicted under codebook_mode=fine
K_FINE="600"     # real: the flat VQ-600 indices live in z500 with values in [0, 600)
...
[ -f "${QUANTIZER_DIR}/state.pt" ]               || { echo "Missing quantizer: ${QUANTIZER_DIR}/state.pt. Run 20_train_quantizer.sh with QUANTIZER_TYPE=vq K1=600 first."; exit 1; }
...
PORT_BASE=$((36000 + (SLURM_JOB_ID % 2500)))

# ==========================================================
# Stage A: Generate VQ targets (single-stream)
# ==========================================================
if [ ! -f "${TARGETS_DIR}/targets.pt" ]; then
    log_phase "Stage A: generate VQ targets (quantizer=${QUANTIZER_TYPE} K=600, single-stream)"
...
# ==========================================================
# Stage C: SSL pretrain on flat VQ-600 targets, codebook-mode=fine, K_f=600
# ==========================================================
...
# ==========================================================
# Stage D: CTC fine-tune (10h, seed 42)
# ==========================================================
...
log_phase "DONE VQ smoke (${QUANTIZER_TYPE} K=600)"
