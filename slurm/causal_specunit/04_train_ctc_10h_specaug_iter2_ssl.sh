#!/bin/bash
# Fine-tune CTC on a 10h labeled subset from an iter-2 SSL pretrained checkpoint.

#SBATCH --partition=common
#SBATCH --qos=bg-eng-01
#SBATCH --account=bg-eng-01
#SBATCH --job-name=csu_ctc10h_i2_ssl
...
#SBATCH --mem=160G
#SBATCH --gres=gpu:2
#SBATCH -o /valhalla/projects/bg-eng-01/LoreaEnc/logs/csu_ctc10h_i2_ssl.%j.out
#SBATCH -e /valhalla/projects/bg-eng-01/LoreaEnc/logs/csu_ctc10h_i2_ssl.%j.err
...
SSL_CHECKPOINT="${SSL_CHECKPOINT:-outputs/causal_specunit/pretrain_ssl_iter2_100k_c8/checkpoint_step100000/checkpoint.pt}"
...
# Mirrors the fair 10h iter-1 recipe so iter-2 can be compared directly.
...
if [ ! -f "${SSL_CHECKPOINT}" ]; then
    echo "Missing iter-2 SSL checkpoint: ${SSL_CHECKPOINT}"
...
echo "Job ${SLURM_JOB_ID} iter-2 10h SpecAug CTC fine-tuning starting at $(date)"
echo "Python: $(which python)"
echo "Torchrun: $(which torchrun)"
echo "Data root: ${DATA_ROOT}"
echo "Iter-2 targets: ${TARGETS_DIR}"
...
echo "Job ${SLURM_JOB_ID} iter-2 10h SpecAug CTC fine-tuning finished at $(date)"
