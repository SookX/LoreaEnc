...
#SBATCH --job-name=csu_purity_ns
#SBATCH --time=06:00:00
#SBATCH --nodes=1
#SBATCH --ntasks-per-node=1
#SBATCH --cpus-per-task=8
#SBATCH --mem=32G
#SBATCH -o /valhalla/projects/bg-eng-01/LoreaEnc/logs/csu_purity_ns.%j.out
#SBATCH -e /valhalla/projects/bg-eng-01/LoreaEnc/logs/csu_purity_ns.%j.err
...
OUTPUT_PATH="outputs/causal_specunit/phone_purity_c8_no_silence.npz"
...
echo "Started: $(date)"
echo "Time left: $(squeue -h -j "${SLURM_JOB_ID}" -o "%L" || true)"
...
if [ ! -d "${TEXTGRID_DIR}" ]; then
    ...
    echo "Submit with: TEXTGRID_DIR=/actual/path sbatch slurm/causal_specunit/01c_phone_purity_no_silence.sh"
    exit 1
fi
