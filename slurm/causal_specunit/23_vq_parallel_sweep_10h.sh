#!/bin/bash
# Parallel-VQ 9M sweep at 10h budget only.
#
# Array of 6 cells = {iter-1, iter-2} x {seed 42, 43, 44} on Libri-Light 10h.
...
#
# Submit:
#   sbatch slurm/causal_specunit/23_vq_parallel_sweep_10h.sh

#SBATCH --partition=common
#SBATCH --qos=bg-eng-01
#SBATCH --account=bg-eng-01
#SBATCH --job-name=vq_p_10h
#SBATCH --array=0-5
...
#SBATCH --mem=128G
#SBATCH --gres=gpu:2
#SBATCH -o /valhalla/projects/bg-eng-01/LoreaEnc/logs/vq_p_10h.%A_%a.out
#SBATCH -e /valhalla/projects/bg-eng-01/LoreaEnc/logs/vq_p_10h.%A_%a.err
...
SUBSET="librilight_10h"
FT_BATCH_SIZE=64   # 2 GPUs * 64 = 128 effective, matches script 10's 10h cell
...
log_phase "CELL ${IDX} | 10h iter=${ITER} seed=${SEED} batch=${FT_BATCH_SIZE} (eff. $((FT_BATCH_SIZE * NPROC_PER_NODE)))"
...
PORT_BASE=$((41000 + (SLURM_JOB_ID % 2500) + IDX * 7))
...
log_phase "DONE 10h iter=${ITER} seed=${SEED}"
