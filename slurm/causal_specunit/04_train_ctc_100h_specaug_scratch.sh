...
#SBATCH --job-name=csu_ctc100h_aug_scratch
...
#SBATCH --mem=256G
#SBATCH --gres=gpu:2
#SBATCH -o /valhalla/projects/bg-eng-01/LoreaEnc/logs/csu_ctc100h_aug_scratch.%j.out
#SBATCH -e /valhalla/projects/bg-eng-01/LoreaEnc/logs/csu_ctc100h_aug_scratch.%j.err
...
OUTPUT_DIR="${OUTPUT_DIR:-outputs/causal_specunit/ctc_scratch_100h_specaug_w10_p50_100ep_c8}"
...
echo "Job ${SLURM_JOB_ID} 100h scratch SpecAug run starting at $(date)"
...
echo "Schedule: lr=${SCRATCH_LR} warmup=${WARMUP_EPOCHS} hold=${PEAK_EPOCHS} decay=${NOAM_DECAY_RATE}"
...
echo "Job ${SLURM_JOB_ID} 100h scratch SpecAug run finished at $(date)"
