...
class Wav2Vec2CollateFunctionForPreTraining:
    """Callable collate class (not a closure) so it can be pickled by Windows multiprocessing."""
    ...
    def __call__(self, batch):
        ...
        ### Store Batch ###
        return {"input_values": audios.unsqueeze(1),
...
if __name__ == "__main__":
    ...
    dataset = LibriSpeechDataset(path_to_data, include_splits=["dev-clean"])
    config = Wav2Vec2Config()

    loader = DataLoader(dataset, batch_size=4, collate_fn=Wav2Vec2CollateFunctionForPreTraining(config))
    next(iter(loader))