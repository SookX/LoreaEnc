...
The gains decrease as labeled data grows --- a well-documented
diminishing-returns pattern for SSL on speech~\citep{wav2vec2,hubert}.
At 960h, where the scratch model already receives dense transcript
supervision over the full LibriSpeech training set and learns strong
acoustic representations directly from the CTC objective, both SSL
initializations slightly underperform scratch, and the iter-2
refinement provides no benefit over iter-1 at this budget. This
crossover behavior --- visible in \cref{fig:wer-vs-hours} --- confirms
that the value of spectrogram-domain SSL is concentrated in the
low- and mid-resource regimes, where pretraining is most consequential.
The decisive contribution is therefore label efficiency: in the 10h
regime, our recipe reduces test-clean WER by $39.2$ absolute points
relative to scratch. We treat the 960h scratch model as a high-resource
supervised reference for the labeled-data axis, not as the primary
...
we attribute this to the diminishing value of cluster-refinement when
the downstream supervised signal alone is sufficient to learn
