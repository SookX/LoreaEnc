...
\begin{abstract}
Automatic speech recognition (ASR) systems depend on large amounts of labeled data, a constraint that self-supervised pretraining (SSL) aims to alleviate. While existing SSL methods such as wav2vec~2.0 and HuBERT achieve state-of-the-art performance, they operate on raw waveforms and require thousands of GPU-hours of pretraining, making them impractical in compute-constrained and edge-deployment settings. In this paper, we reformulate self-supervised speech representation learning to operate entirely in the spectrogram domain, using a HuBERT-style masked-prediction objective over dual-codebook discrete units derived from short log-mel chunks. Applied to a 9M-parameter SqueezeFormer-XS encoder, our recipe reduces test-other WER from 80.1\% to 49.8\% (30.3 absolute, 38\% relative) when fine-tuned on 10 hours of labeled data, and from 55.1\% to 39.1\% (16.0 absolute, 29\% relative) on 100 hours --- at \emph{42 GPU-hours} of total pretraining, roughly $\mathbf{50\times}$ less than wav2vec~2.0 Base.
\end{abstract}
...
\begin{figure}[t!]
  ...
  \begin{tikzpicture}
    \begin{semilogxaxis}[
      ...
      % --- Scratch baseline (placeholder values) ---
      \addplot[color=plotScratch, dashed, mark=square*,
               mark options={fill=white, draw=plotScratch}]
        coordinates {(10, 80.1) (100, 55.1) (960, 13.4)};
      \addlegendentry{Scratch}

      % --- SSL iter-1 (placeholder values) ---
      \addplot[color=plotIter1, mark=triangle*]
        coordinates {(10, 53.6) (100, 44.4) (960, 15.6)};
      \addlegendentry{+ SSL (iter-1)}

      % --- SSL iter-2 (placeholder values) — highlighted as the result ---
      \addplot[color=plotIter2, mark=*, line width=1.6pt]
        coordinates {(10, 49.8) (100, 39.1) (960, 14.0)};
      \addlegendentry{\textbf{+ SSL (iter-2, ours)}}

      % --- Inline data labels on the iter-2 line: pinned above each marker
      % with a white background so they "punch through" any nearby line.
      \node[font=\scriptsize\bfseries, plotIter2, fill=white, inner sep=1.2pt,
            anchor=south, yshift=4pt]
        at (axis cs:10,49.8)  {49.8};
      \node[font=\scriptsize\bfseries, plotIter2, fill=white, inner sep=1.2pt,
            anchor=south, yshift=4pt]
        at (axis cs:100,39.1) {39.1};
      \node[font=\scriptsize\bfseries, plotIter2, fill=white, inner sep=1.2pt,
            anchor=south, yshift=8pt]
        at (axis cs:960,14.0) {14.0};
  ...
  \caption{Test-other WER vs.\ labeled fine-tuning data on LibriSpeech.
    ...
    low-resource regime (shaded band). \emph{Values are placeholders
    matching Table~\ref{tab:main}; final numbers will replace these in
    the camera-ready.}}
...
In this work, we present a three-stage spectrogram-only SSL pipeline that combines log-mel chunk clustering with masked-prediction pretraining of a compact SqueezeFormer encoder. Targets are produced by PCA-reduced $K$-means over short log-mel chunks at two granularities ($K_c{=}100$ coarse, $K_f{=}500$ fine), and the encoder is pretrained on LibriSpeech 960h to predict both codebooks at masked positions before being fine-tuned with CTC for ASR. The recipe yields consistent WER improvements over from-scratch baselines in the 10h and 100h fine-tuning regimes at roughly $\mathbf{50\times}$ less pretraining compute than waveform-based SSL, and we ablate the contribution of each codebook to validate the dual-vocabulary design.

\paragraph{Contributions.}
(i) A simple, teacher-free, spectrogram-domain SSL recipe that pretrains a 9M-parameter encoder in 42 GPU-hours --- two orders of magnitude less than waveform-based SSL baselines.
(ii) A dual-codebook objective ($K_c{=}100$ + $K_f{=}500$) that we show outperforms either codebook alone via a controlled ablation.
(iii) Empirical characterization of the labeled-data regime where compact-model spectrogram SSL is beneficial (10h, 100h) and where it diminishes (960h), consistent with the well-documented SSL scaling behavior on speech.
...
acoustic features in an offline pass; unlike HuBERT, however, we operate
\emph{entirely in the spectrogram domain} and avoid both the MFCC
front-end and the iterative refinement loop, trading a small amount of
target quality for a substantially cheaper pretraining setup.
...
within-category detail. We pretrain with AdamW on LibriSpeech 960h for
$150$k optimizer steps; full hyperparameters are listed in
Sec.~\ref{sec:setup}.
...
budgets: 10h (\textit{train-clean-10h}), 100h (\textit{train-clean-100}),
and the full 960h. The baseline is a SqueezeFormer-XS CTC model trained
from scratch for 150 epochs on each split; our method initializes the same
SqueezeFormer from a spectrogram-domain SSL checkpoint and is fine-tuned
for 150 epochs under an \emph{otherwise identical} recipe (same optimizer,
schedule, batch size, and CTC loss). All numbers are greedy CTC decoding
on \textit{test-clean} / \textit{test-other} without an external language
model.
    ...
    \begin{small}
      \begin{sc}
        \begin{tabular}{lcccccc}
          ...
          \midrule
          SqueezeFormer-XS (scratch)          & x.xx                    & 80.1                     & x.x                      & 55.1          & x.xx           & xx.x          \\
          \quad + SSL (iter-1)                & x.xx                    & 53.6                     & x.x                      & 44.4          & x.xx           & xx.x          \\
          \quad \textbf{+ SSL (iter-2, ours)} & \textbf{x.xx}           & \textbf{49.8}            & \textbf{x.x}             & \textbf{39.1} & \textbf{x.x}  & \textbf{x.xx} \\
          \bottomrule
        \end{tabular}
      \end{sc}
    \end{small}
...
labeled-data axis. Our SSL-pretrained SqueezeFormer improves over the
from-scratch baseline at every low-resource budget, with the largest
relative gain in the 10h regime. A second iteration of cluster
refinement --- where targets are re-derived from the pretrained
encoder's hidden states rather than raw log-mel chunks --- yields a
further improvement, mirroring the iter-1/iter-2 gap reported by
HuBERT~\citep{hubert}.

The gains decrease as the amount of labeled data grows --- a
well-documented diminishing-returns pattern for SSL on
speech~\citep{wav2vec2,hubert}. In the 960h regime, the scratch model
already receives dense transcript supervision over the full LibriSpeech
training set and learns strong acoustic representations directly from
the CTC objective. The decisive contribution of spectrogram-domain SSL
is therefore label efficiency: in the 10h regime, where pretraining
matters most, our recipe reduces test-other WER by 30.3 absolute points
relative to scratch. We treat the 960h scratch model as a
high-resource supervised reference for the labeled-data axis, not as the
primary target of the method. For compact models, this crossover regime
between SSL benefit and supervised saturation has not, to our knowledge,
been previously characterized at this model scale.
...
we compute frame-level phone purity~\citep{hsu2017hubert-phone} against
forced-aligned LibriSpeech phoneme labels. $K_c{=}100$ achieves
$\mathbf{XX.X\%}$ purity over 39 phones (random baseline: $\sim$2.6\%);
$K_f{=}500$ achieves $\mathbf{YY.Y\%}$. Both substantially exceed chance,
confirming that the clusters encode phonetic structure despite being
derived directly from log-mel chunks without any forced alignment or
acoustic-model bootstrapping.\footnote{Computed with
\texttt{evaluate\_phone\_purity.py} on \textit{train-clean-100} using
the publicly released LibriSpeech MFA alignments; silence and noise
intervals excluded.}
...
directly at 8.4 wall-hours per 100k steps on 2 GPUs; iter-2 was timed
identically. Total GPU-hours follow as $2{\times}21$.} Under this
accounting, the spectrogram-domain recipe pretrains in two orders of
magnitude less compute than the waveform-based SSL references.
